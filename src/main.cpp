...
uint256 hashGenesisBlock("0xee960feb663ca4231c851e0ef84205ec068a5efec38bcff39d83a6b637039449");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Aiden: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Aiden Signed Message:\n";
...
{
    // Aiden: IsDust() detection disabled, allows any valid dust to be relayed.
...
const unsigned char TheNfactor = 5;
...
static const int64 nTargetTimespan = 3.5 * 24 * 60 * 60; // Aiden: 3.5 days
static const int64 nTargetSpacing = 2.5 * 60; // Aiden: 2.5 minutes
...
unsigned int static GetNextWorkRequired_V1(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Aiden: This fixes an issue where a 51% attack can change difficulty at will.
...
{
        static const int64        BlocksTargetSpacing                        = 0.5 * 60; // 2.5 minutes
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Aiden: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Aiden: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x9b37e368684f25a05a440287990bd53f9d1be3cced840d2052c2533d764a03d6");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        block.nNonce = 1390771;
        block.nTime = 1396615615;
        
        if (fTestNet)
        {

            block.nNonce = 103293;
            block.nTime = 1396616971;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0xb82a06c6261cb11ad8332da805bcc356bf120ffecb94a2dd9e7f2c7ebecdf930"));
...
unsigned char pchMessageStart[4] = { 0xde, 0xfa, 0xce, 0xed }; // Aiden: increase each by adding 1 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("AidenMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("AidenMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static AidenMiner(CWallet *pwallet)
{
    printf("AidenMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("aiden-miner");
    ...
    try { while (true) {
        ...
        printf("Running AidenMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&AidenMiner, pwallet));
}
